...
export default function Category() {
  ...
  const description = `${category} | Sapna Shri Jewellers`;
  const baseURL = "https://sapnashrijewellers.github.io";
  const driveURL = `${baseURL}/static/img/optimized/`;
  const imageUrl = `${driveURL}${filtered[0].images[0]}`;
  renderSEOTags(title, description, imageUrl, `${baseURL}/category/${category}`);
