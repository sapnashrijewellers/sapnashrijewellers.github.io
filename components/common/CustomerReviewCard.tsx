import React from 'react';
import { Star, BadgeCheck } from 'lucide-react';
import { CustomerReview } from '@/types/catalog';

const MAX_STARS = 5;
const MIN_STARS = 1;
const STAR_ARRAY = Array.from({ length: MAX_STARS }, (_, idx) => idx + 1);

export interface CustomerReviewCardProps {
  review: CustomerReview;
  className?: string;
  datePublished?: string;
}

export const CustomerReviewCard: React.FC<CustomerReviewCardProps> = ({ review, className = '', datePublished }) => {
  const { name, rating, text, isVerified = true } = review;

  const clampedRating = Math.max(MIN_STARS, Math.min(MAX_STARS, Math.round(rating)));

  return (
    <article
      className={`bg-surface border-theme flex w-[calc(100vw-2rem)] max-w-[360px] shrink-0 snap-start flex-col justify-between rounded-2xl border p-4 shadow-sm sm:w-[340px] sm:p-5 ${className}`.trim()}
    >
      <div>
        {/* Rating Header */}
        <div className="mb-3 flex items-center justify-between gap-2">
          <div
            className="flex items-center gap-0.5"
            role="img"
            aria-label={`Customer rating: ${clampedRating} out of ${MAX_STARS} stars`}
          >
            {STAR_ARRAY.map((starIndex) => (
              <Star
                key={starIndex}
                className={`h-3.5 w-3.5 ${
                  starIndex <= clampedRating ? 'fill-amber-500 text-amber-500' : 'text-muted-foreground/30'
                }`}
                aria-hidden="true"
              />
            ))}
          </div>

          {isVerified && (
            <span className="inline-flex items-center gap-1" aria-label="Verified customer">
              <BadgeCheck className="h-3.5 w-3.5 shrink-0 text-sm font-bold text-emerald-800" aria-hidden="true" />
              <span className="text-xs">Verified Customer</span>
            </span>
          )}
        </div>

        {/* Testimonial */}
        <blockquote className="text-foreground/90 mb-4 text-sm leading-relaxed italic">&ldquo;{text}&rdquo;</blockquote>

        {datePublished && (
          <time dateTime={datePublished} className="sr-only">
            {datePublished}
          </time>
        )}
      </div>

      {/* Author */}
      <footer className="text-foreground/80 border-theme/30 border-t pt-2.5 text-xs font-semibold">
        <cite className="not-italic">
          <span className="sr-only">Testimonial from</span>— {name}
        </cite>
      </footer>
    </article>
  );
};

export default CustomerReviewCard;
