...
export default function FeaturedJewellery() {
  ...
  const activeLabel = useMemo(() => merchandisingLabels.find((label) => label.key === activeKey), [activeKey]);
  ...
  const loadProducts = async (label: Label) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load ${label.key}.json`);
      }
      ...
      console.error(`Unable to load products for ${label.name}`, error);
  ...
  return (
    <section aria-labelledby="merchandising-heading" className="w-full">
        ...
        {merchandisingLabels.map((label) => {
          ...
          return (
            <button
              ...
              className={`focus-visible:ring-primary/40 inline-flex shrink-0 snap-start items-center gap-2 rounded-full px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-all duration-200 focus:outline-none focus-visible:ring-2 ${
                isActive
                  ? 'bg-accent shadow-sm'
      ...
      {/* Active collection */}
      <div role="tabpanel" id={`panel-${activeKey}`} aria-labelledby={`tab-${activeKey}`} tabIndex={0} className="mt-3">
        {/* Product carousel */}
        <div
          role="region"
          aria-label={`${activeLabel?.collectionName ?? ''} jewellery carousel`}
          tabIndex={0}
          className="scrollbar-hide focus:ring-primary/40 flex snap-x snap-mandatory gap-3 overflow-x-auto rounded-2xl p-2 focus:ring-1 focus:outline-none sm:gap-4"
        >
          {activeProducts.map((product, index) => (
            <ProductCard
              ...
              key={product.id}
              priority={index < 4}
              className="w-[160px] shrink-0 snap-start transition-transform duration-150 ease-out will-change-transform sm:w-[180px] lg:w-[220px]"
            />
          ))}
        </div>
      </div>
